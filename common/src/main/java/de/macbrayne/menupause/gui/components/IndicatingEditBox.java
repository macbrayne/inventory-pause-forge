    ...
    public IndicatingEditBox(Font font, int width, int height) {
        ...
        this.setResponder(s -> {
            ...
            if(status != ClassStatus.OK) {
                ...
                ((MutableTooltip) warning.getTooltip()).menupause$updateMessage(Minecraft.getInstance(), Component.translatable(translation));
            } else {
                ((MutableTooltip) warning.getTooltip()).menupause$updateMessage(Minecraft.getInstance(), Component.empty());
            }
        });
    }
